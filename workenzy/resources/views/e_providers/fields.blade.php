...
<!-- Name Field -->
    ...
    <!-- Country Id Field -->
    <div class="form-group align-items-baseline d-flex flex-column flex-md-row">
        {!! Form::label('country_id', trans("lang.country"),['class' => 'col-md-3 control-label text-md-right mx-1']) !!}
        <div class="col-md-9">
                @php
                    $country_to_translate = [];
                    foreach($country as $k => $v)
                    {
                        $country_to_translate[$k] = trans('lang.'.str_replace(" ","_",$v));
                    }
                @endphp
                {!! Form::select('country_id', $country_to_translate, null, ['class' => 'select2 form-control']) !!}
            <div class="form-text text-muted">{{ trans("lang.country_name_help") }}</div>
        </div>
    </div>
